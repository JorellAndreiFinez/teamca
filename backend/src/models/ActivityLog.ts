...
const activityLogSchema = new mongoose.Schema<IActivityLog>(
  {
    ...
    action_type: { type: String, required: true, enum: ["create", "read", "update", "delete", "login", "logout"] },
    resource_type: { type: String, required: true, enum: ["user", "task", "department", "dtr", "internProfile", "auth"] },
