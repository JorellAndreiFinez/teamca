...
export const timeAdjustmentController = {
  ...
  async getPendingRequests(req: Request, res: Response) {
    try {
      ...
      const departmentId = !isAdmin ? user.departments?.[0]?.department_id?.toString() : undefined;

      const requests = await timeAdjustmentService.getPendingRequests(departmentId);
  ...
  async approveRequest(req: Request, res: Response) {
    try {
      ...
      if (!isAdmin && !isHead) {
        return res
          .status(403)
          .json({ success: false, message: "Unauthorized to approve requests" });
      }
  ...
  async rejectRequest(req: Request, res: Response) {
    try {
      ...
      if (!validated.reviewNotes) {
        return res
          .status(400)
          .json({ success: false, message: "Review notes are required for rejection" });
      }
